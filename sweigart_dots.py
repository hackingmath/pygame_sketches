...
def dots(x,y,t,number_of_dots):
    for i in range(number_of_dots):
        angle = i * pi / number_of_dots
        r = 270 * sin(t * 0.05 + angle)
        pygame.draw.line(screen, CYAN, [x - 270*cos(angle), y-270*sin(angle)],
                                        [x + 270*cos(angle), y+270*sin(angle)], 2)
    for i in range(number_of_dots):
        angle = i * pi / number_of_dots
        r = 270 * sin(t * 0.05 + angle)
        pygame.draw.circle(screen, VIOLET, [x + r*cos(angle), y+r*sin(angle)], 15)
